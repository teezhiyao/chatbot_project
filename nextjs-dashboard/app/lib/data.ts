...
export async function fetchChatLogs() {
  try {
    const data = await sql<ChatMessage>`SELECT * FROM Messages`;
...
export async function sendMessage(message: ChatMessage) {
  try {
    ...
    // );
    console.log(message);
    sql<ChatMessage>`INSERT INTO Messages (chat_id, sender_id, content) 
      VALUES (${message.chat_id}, ${message.sender_id}, ${message.content}) ON CONFLICT (message_id) DO NOTHING;`;
  } catch (error) {
    console.error("Database Error:", error);
